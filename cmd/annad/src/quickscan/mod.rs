use crate::advice;
use crate::advice::types::{AdvicePlan, AdviceSeverity};
...
static QUICKSCAN_DIR: OnceLock<PathBuf> = OnceLock::new();

pub fn set_quickscan_dir(path: PathBuf) {
    QUICKSCAN_DIR.set(path).ok();
}

fn quickscan_dir() -> &'static PathBuf {
    QUICKSCAN_DIR.get().expect("quickscan dir not initialized")
}
...
pub async fn run(config: Arc<Config>) -> Result<QuickscanReport> {
    ...
    let retain = config.quickscan.retain_reports.max(1);
    persist_report(&report, retain)?;
...
pub async fn run_initial_if_needed(config: Arc<Config>) -> Result<Option<QuickscanReport>> {
    if has_recent_report(time::Duration::hours(24))? {
        info!(
            target: "annad",
            "skip initial quickscan; recent report within 24h"
        );
        return Ok(None);
    }
    run(config).await.map(Some)
}

pub fn prune_report_cache(retain: usize) -> Result<()> {
    prune_reports(retain.max(1))
}

/// Run quickscan for a specific user and persist to their per-user directory
pub async fn run_for_user(
    config: Arc<Config>,
    reports_dir: &Path,
    advice_dir: &Path,
) -> Result<QuickscanReport> {
    if !config.quickscan.enable {
        return Err(anyhow!("quickscan disabled"));
    }
    ...
    info!(
        target: "annad",
        "quickscan for user completed in {:.2}s",
        elapsed.as_secs_f64()
    );
    ...
    // Persist to per-user reports directory
    persist_report_to(&report, reports_dir, config.quickscan.retain_reports)?;

    // Seed advice for this user
    seed_advice_to(&findings, advice_dir)?;

    Ok(report)
}

/// Persist report to custom directory (per-user)
fn persist_report_to(report: &QuickscanReport, reports_dir: &Path, retain: usize) -> Result<()> {
    ensure_dir(reports_dir, 0o2770)?; // setgid for group ownership
    let generated = OffsetDateTime::parse(&report.generated, &Rfc3339)
    ...
    let dir = reports_dir.join(stamp);
    ensure_dir(&dir, 0o2770)?;
    let json_path = dir.join("quickscan.json");
    let txt_path = dir.join("quickscan.txt");

    write_secure(&json_path, &serde_json::to_vec_pretty(report)?, 0o660)?;
    ...
    write_secure(&txt_path, txt.as_bytes(), 0o660)?;
    prune_reports_in(reports_dir, retain)?;

    Ok(())
}

/// Seed advice to custom directory (per-user)
fn seed_advice_to(findings: &[Finding], advice_dir: &Path) -> Result<()> {
    for finding in findings {
        if let Some(fix) = &finding.fix {
            ...
            let severity = match finding.severity {
                Severity::Info => AdviceSeverity::Info,
                Severity::Warn => AdviceSeverity::Warn,
                Severity::Action => AdviceSeverity::Action,
            };
            advice::create_quickscan_advice_in(
                &finding.id,
                &finding.summary,
                severity,
                plan,
...
/// Prune old reports in a custom directory
fn prune_reports_in(reports_dir: &Path, retain: usize) -> Result<()> {
    ...
    let mut entries = Vec::new();
    for entry in
        fs::read_dir(reports_dir).with_context(|| format!("read dir {}", reports_dir.display()))?
    {
        let entry = entry.with_context(|| "read report entry".to_string())?;
        if !entry
            .file_type()
        ...
        let path = entry.path();
        let json_path = path.join("quickscan.json");
        if !json_path.exists() {
            continue;
        }
    ...
    for (_, path) in entries.iter().skip(keep) {
        if let Err(err) = fs::remove_dir_all(path) {
            warn!(
                target: "annad",
                "failed removing old report {}: {err}",
                path.display()
            );
        }
    }
...
fn persist_report(report: &QuickscanReport, retain: usize) -> Result<()> {
    let root = quickscan_dir();
    ...
    write_secure(&json_path, &serde_json::to_vec_pretty(report)?, 0o600)?;
    ...
    write_secure(&txt_path, txt.as_bytes(), 0o600)?;
    prune_reports(retain)?;
...
fn seed_advice(findings: &[Finding]) -> Result<()> {
    for finding in findings {
        if let Some(fix) = &finding.fix {
            ...
            advice::create_quickscan_advice(&finding.id, &finding.summary, severity, plan)?;
        }
    }
    Ok(())
}
...
fn write_secure(path: &Path, data: &[u8], mode: u32) -> Result<()> {
    if let Some(parent) = path.parent() {
        ensure_dir(parent, 0o2770)?;
    }
...
fn prune_reports(retain: usize) -> Result<()> {
    let keep = retain.max(1);
    let root = quickscan_dir();
    if !root.exists() {
        return Ok(());
    ...
    for (_, path) in entries.into_iter().take(excess) {
        fs::remove_dir_all(&path)
            .with_context(|| format!("remove stale report {}", path.display()))?;
    }
    Ok(())
}

fn latest_report_generated() -> Result<Option<OffsetDateTime>> {
    let root = quickscan_dir();
    if !root.exists() {
        return Ok(None);
    }
    ...
    for entry in fs::read_dir(root).with_context(|| format!("read dir {}", root.display()))? {
        ...
        let json_path = entry.path().join("quickscan.json");
        if !json_path.exists() {
            continue;
        }
        let modified = json_path
        ...
        match &latest {
            ...
            _ => latest = Some((modified, json_path)),
        }
    }
    ...
    let report: QuickscanReport = match serde_json::from_str(&contents) {
        Ok(rep) => rep,
        Err(err) => {
            warn!(
                target: "annad",
    ...
    match OffsetDateTime::parse(&report.generated, &Rfc3339) {
        Ok(dt) => Ok(Some(dt)),
        Err(err) => {
            warn!(
                target: "annad",
                "invalid report timestamp {}: {err}",
                report.generated
            );
            Ok(None)
        }
...
fn has_recent_report(max_age: time::Duration) -> Result<bool> {
    if max_age.is_zero() {
        return Ok(false);
    }
    if let Some(latest) = latest_report_generated()? {
...
async fn memory_swap_probe() -> Finding {
    match collect_swap_snapshot().await {
        Ok(snapshot) => {
            ...
            let mut severity = if headroom_bytes == 0 || headroom_ratio < action_threshold {
                Severity::Action
...
async fn storage_probe() -> Finding {
    ...
    let summary = match severity {
        ...
        Severity::Warn | Severity::Action => {
            if let Some(top) = issues.iter().find(|issue| issue.severity == severity) {
