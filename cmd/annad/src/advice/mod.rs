...
use types::{Advice, AdvicePlan, AdviceSeverity};
use which::which;
...
pub fn create_quickscan_advice(
    id_suffix: &str,
    summary: &str,
    ...
    let stamp = hour_bucket
        .format(&format_description!("[year][month][day]T[hour][minute]00Z"))
        .unwrap_or_else(|_| "00000000T000000Z".to_string());
    let sanitized_suffix = id_suffix.replace('.', "-");
    let advice = Advice {
        id: format!("quickscan-{}-{}", sanitized_suffix, stamp),
...
/// Create quickscan advice in a custom directory (per-user)
pub fn create_quickscan_advice_in(
    id_suffix: &str,
    summary: &str,
    severity: AdviceSeverity,
    plan: AdvicePlan,
    ...
    fs::write_advice_to(&advice, advice_dir)?;
    Ok(())
}
