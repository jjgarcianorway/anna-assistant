...
static SIGNALS_DIR: OnceLock<PathBuf> = OnceLock::new();

pub fn set_signals_dir(path: PathBuf) {
    SIGNALS_DIR.set(path).ok();
}

fn signals_dir() -> &'static PathBuf {
    SIGNALS_DIR.get().expect("signals dir not initialized")
}
