...
pub fn run(args: PersonaArgs<'_>, cfg: &UiCfg, style: &Style) -> Result<()> {
    match args.command {
        ...
        PersonaCommand::Debug => debug(cfg, style, args.raw),
    }
}
...
fn debug(cfg: &UiCfg, style: &Style, raw: bool) -> Result<()> {
    use crate::paths::AnnaPaths;
    use crate::rpc::RpcClient;

    // Print warning banner (not in raw mode)
    if !raw {
        println!("\n{}", ui::err(style, "⚠ INTERNAL PERSONA DEBUG"));
        println!(
            "{}",
            ui::warn(
                style,
                "Personas are internal by default and not shown to users in normal operation."
            )
        );
        println!(
            "{}",
            ui::dim_gray(style, "This command is for development/debugging only.\n")
        );
    }
    ...
    let request = anna_rpc::Request::PersonaSummary(anna_rpc::PersonaSummaryRequest { uid });

    match client.call(request) {
        Ok(anna_rpc::Response::PersonaSummary(response)) => {
            if raw {
                println!(
                    "{}",
            ...
            println!("{}", ui::head(style, "Persona Debug Information"));
            println!("{}", ui::kv(style, "Persona", &response.persona));
            println!("{}", ui::kv(style, "Source", &response.source));
            println!(
                "{}",
                ui::kv(style, "Confidence", &format!("{:.2}", response.confidence))
            );

            if !response.traits.is_empty() {
                println!("\n{}", ui::head(style, "Traits"));
                for trait_info in &response.traits {
                    let confidence_str = format!("{:.2}", trait_info.confidence);
                    println!(
                        "{}",
                        ui::kv(
                            style,
                    ...
                    if !trait_info.evidence.is_empty() {
                        for evidence in &trait_info.evidence {
                            println!("  {}", ui::dim_gray(style, &format!("• {}", evidence)));
                        }
                    }
                }
            } else {
                println!(
                    "\n{}",
                    ui::warn(style, "No traits detected (samplers not yet run)")
                );
            }
        ...
        Ok(anna_rpc::Response::Error(err)) => {
            Err(PersonaCliError::other(format!("Error: {}", err.message)))
        }
        Ok(_) => Err(PersonaCliError::other("Unexpected response type")),
        Err(e) => Err(PersonaCliError::other(format!(
            "Failed to fetch persona debug info: {}",
            e
        ))),
    }
}
