...
### 0. Bundles Ignore User Telemetry - FUNDAMENTAL FLAW
**Severity:** CRITICAL
**Status:** 🔴 BROKEN

**Problem:**
Bundle installs packages blindly without checking what user already has/uses:
```
Will install 16 item(s):
  1. Install nano text editor           ← USER ALREADY HAS VIM!
```

**User Quote:**
> "this list must be dynamic based on the user telemetry... what is the point of installing nano when I have vim? (and if you check CPU user time or command history... you would see that I use it!!"

**Impact:**
- Defeats entire purpose of intelligent recommendations
- Wastes time installing unwanted packages
- Shows Anna doesn't understand user's system
- Breaks trust in Anna's intelligence

**Fix Required:**
1. **Check existing packages** before recommending:
   - If user has vim/neovim/emacs → DON'T install nano
   - If user has mpv → DON'T install vlc
   - If user has Firefox → DON'T install Chrome

2. **Check actual usage** (telemetry):
   - Command history (which editor they run)
   - CPU time (which apps they use most)
   - Recently used packages

3. **Smart recommendations**:
   ```
   Text Editor:
     ✓ You already use vim (40% CPU time)
     Would you like to enhance vim?
       • vim-plug (plugin manager)
       • fzf.vim (fuzzy finder)
       • vim-airline (status bar)
   ```

4. **Offer alternatives** intelligently:
   ```
   Alternative Editors:
     • neovim (modern vim fork)
     • helix (rust-based vim-like)

   [Skip this category] [Show all alternatives]
   ```

**Implementation:**
- Bundles must query telemetry BEFORE generating package list
- Filter out packages user already has and uses
- Only suggest genuinely complementary packages
- Allow user to see and customize before applying
...
**Problem:**
- Daemon crashed but `annactl doctor` reports "✓ Anna daemon running"
...
**Problem:**
```
• 17 suggestions about Utilities
• 10 suggestions about Security & Privacy
• 7 suggestions about Security & Privacy        ← DUPLICATE!
• 9 suggestions about Development Tools
• 5 suggestions about Development Tools         ← DUPLICATE!
• 7 suggestions about System Configuration
• 4 suggestions about System Configuration      ← DUPLICATE!
...
- ✓ Socket file exists and accessible
- ✓ Dependencies satisfied (curl, jq, etc.)
- ✓ Binaries exist and executable
- ✓ Service file correct and loaded
...
2. Prioritize recommendations:
   - If 85% terminal time → prioritize CLI tools
   - If heavy vim usage → prioritize vim improvements
   - If GUI focused → prioritize GUI apps
   - If gaming detected → gaming tools higher priority
...
```
╭──────────────────────────────────────────────────╮
│  Welcome to Hyprland! - Anna Assistant          │
╰──────────────────────────────────────────────────╯
...
**Bundles to perfect:**
- ✓ Hyprland (reference implementation)
- ⏳ GNOME
- ⏳ KDE Plasma
- ⏳ i3
- ⏳ Sway
...
**Must Have:**
- ✓ Daemon stable and auto-recovers
- ✓ All commands respond in <1 second
- ✓ Doctor accurately detects Anna problems
- ✓ <50 recommendations max
- ✓ Telemetry-based prioritization working
- ✓ Clear separation: report vs advise
