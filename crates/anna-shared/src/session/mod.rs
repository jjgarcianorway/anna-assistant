...
impl UserPreferences {
    ...
    pub fn update_from_signal(&mut self, signal: &str) {
        ...
        if sig.contains("be brief") || sig.contains("shorter") || sig.contains("concise") {
            ...
            changed = true;
        } else if sig.contains("explain more") || sig.contains("more detail") || sig.contains("elaborate") {
...
impl ContradictionStore {
    ...
    pub fn record(&mut self, claim_type: &str, wrong: &str, correct: &str, source_cmd: &str) {
        ...
        if let Some(existing) = self.patterns.iter_mut().find(|p|
            p.claim_type == claim_type && p.wrong_assertion == wrong
        ) {
        ...
        // Keep max 100 patterns, remove oldest/least-used
