...
pub fn render_junior_review(attempt: u8, score: u8, verified: bool, issues: &[String]) {
    ...
    print_label(
        "junior",
        &format!("attempt {} -> {} (score={})", attempt, status, score),
        colors::CYAN,
    );
...
pub fn render_revision_applied(changes_made: &[String]) {
    if !changes_made.is_empty() {
        ...
        print_label(
            "revision",
            &format!("{} change{}", changes_made.len(), plural),
            colors::DIM,
        );
...
pub fn render_review_gate(decision: &str, score: u8, requires_llm: bool) {
    ...
    print_label(
        "gate",
        &format!("{} (score={}){}", decision, score, llm_tag),
        colors::CYAN,
    );
}
...
pub fn render_team_review(team: &str, reviewer: &str, decision: &str, issues_count: usize) {
    ...
    print_label(
        &format!("{}/{}", team, reviewer),
        &format!("{}{}", decision, issues_str),
        colors::CYAN,
    );
}
...
pub fn render_clarification_verified(verified: bool, source: &str, alternatives: &[String]) {
    if verified {
        print_label(
            "verify",
            &format!("{}confirmed{} ({})", colors::OK, colors::RESET, source),
            colors::DIM,
        );
    } else {
        print_label(
            "verify",
            &format!("{}not found{} ({})", colors::WARN, colors::RESET, source),
            colors::DIM,
        );
...
pub fn render_fast_path(handled: bool, class: &str, reason: &str, probes_needed: bool) {
    if handled {
        let cache_status = if probes_needed {
            "(probes run)"
        } else {
            "(cached)"
        };
        print_label(
            "fast",
            &format!("{} {} (no LLM needed)", class, cache_status),
            colors::OK,
        );
    } else {
...
pub fn render_evidence_summary(
    ...
    print_label(
        ...
        &format!(
            "{} probe{}, kinds: {:?}",
            probe_count, plural, evidence_kinds
        ),
        colors::DIM,
    );
