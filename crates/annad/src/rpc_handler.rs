...
fn build_editor_config_with_change(
    editor: &str,
) -> (
    String,
    Option<anna_shared::change::ChangePlan>,
    Vec<anna_shared::change::ChangePlan>,
) {
    use anna_shared::change::{plan_ensure_line, plan_ensure_line_with_pattern, ChangePlan};
    use anna_shared::editor_recipes::{get_recipe, ConfigFeature, Editor};
        ...
        if let Some(recipe) = get_recipe(editor_enum, ConfigFeature::SyntaxHighlighting) {
            ...
            for line in &recipe.lines {
                ...
                match plan_result {
                    ...
                    Err(e) => tracing::warn!("Could not plan change for {}: {}", editor, e),
                }
            }
