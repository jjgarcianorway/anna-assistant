...
mod tests {
    use super::*;
    use anna_shared::learning_engine::{RecipePattern, AnswerTemplate, RecipeLogic, LogicType, AnswerKind, RecipeOrigin};

    #[test]
    fn test_find_matching_recipe_by_intent() {
        let mut library = RecipeLibrary::new();

        let mut recipe = LearnedRecipe::new("test-recipe", "system");
        recipe.pattern = RecipePattern::new("check_ram").with_keywords(&["check", "ram"]);
        recipe.enabled = true;
        recipe.answer_template = AnswerTemplate::new("test answer", "test detailed");
        ...
        recipe.origin = RecipeOrigin::learned("test", "Test");

        library.add(recipe).unwrap();
        ...
        let result = find_matching_recipe(&library, &parsed);
        assert!(result.is_some());
        assert_eq!(result.unwrap().id, "test-recipe");
    }

    #[test]
    fn test_find_matching_recipe_by_keywords() {
        let mut library = RecipeLibrary::new();

        let mut recipe = LearnedRecipe::new("test-recipe", "system");
        recipe.pattern = RecipePattern::new("different_intent").with_keywords(&["check", "memory"]);
        recipe.enabled = true;
        recipe.answer_template = AnswerTemplate::new("test answer", "test detailed");
        recipe.logic = RecipeLogic {
            logic_type: LogicType::Template,
