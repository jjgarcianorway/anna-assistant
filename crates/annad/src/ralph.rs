...
/// v0.3.9: Instant response with wiki citation source
struct InstantResponse {
    answer: &'static str,
    wiki_source: &'static str,
    wiki_url: Option<&'static str>,
}

/// v0.2.7: Instant responses for well-known error patterns
/// v0.3.9: Now returns wiki citation sources for grounded answers
/// These are common issues that have known solutions - no need to investigate
fn get_instant_error_response(question: &str) -> Option<InstantResponse> {
...
/// v0.3.9: Now includes wiki citations for grounded answers
fn try_instant_error(question: &str) -> Option<AskResult> {
    ...
    // v0.3.9: Create citation from wiki source
    let citation = anna_shared::rpc::Citation {
        source: response.wiki_source.to_string(),
        url: response.wiki_url.map(|u| u.to_string()),
        section: None,
    };
