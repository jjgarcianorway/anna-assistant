...
fn resolve_probe_command(probe_spec: &str) -> Option<String> {
    ...
    if known_commands
        .iter()
        .any(|&cmd| first_word == cmd || first_word.ends_with(cmd))
        || probe_spec.contains(' ')
        || probe_spec.contains('|')
        || probe_spec.contains('>')
    {
    ...
    warn!(
        "v0.45.6: Unknown probe specifier '{}' - cannot resolve to command",
        probe_spec
    );
    None
}
...
pub async fn run_probes(
    ...
    info!(
        "v0.45.6: Running {} planned probes",
...
mod tests {
    ...
    fn test_resolve_translator_probe_id() {
        ...
        assert_eq!(
            resolve_probe_command("top_memory"),
            Some("ps aux --sort=-%mem".to_string())
        );
    }
    ...
    fn test_resolve_direct_shell_command() {
        ...
        assert_eq!(
            resolve_probe_command("free -b"),
            Some("free -b".to_string())
        );
