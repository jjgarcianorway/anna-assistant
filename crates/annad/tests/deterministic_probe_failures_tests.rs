//! Integration tests for deterministic answerer - Probe failure handling.
...
#[test]
fn test_wrong_probe_command_not_matched() {
    let context = make_context();
    let probes = vec![ProbeResult {
        command: "top -b -n 1".to_string(), // Wrong command
        exit_code: 0,
    ...
    // Should return None because command doesn't match expected pattern
    assert!(answer.is_none());
}
