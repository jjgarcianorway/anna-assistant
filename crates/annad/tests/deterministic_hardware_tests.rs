//! Integration tests for deterministic answerer - Hardware queries.
//!
//! Tests CPU, RAM, and GPU information queries.
...
#[test]
fn test_gpu_info_with_vram() {
    let context = make_context();
    let probes = vec![];

    let answer = deterministic_answerer::try_answer("what gpu do i have?", &context, &probes);

    assert!(answer.is_some());
    let answer = answer.unwrap();
    assert!(answer.contains("NVIDIA"));
    assert!(answer.contains("8.0 GB VRAM"));
}

#[test]
fn test_gpu_info_without_vram() {
    let mut context = make_context();
    context.hardware.gpu_vram_gb = None;
    let probes = vec![];

    let answer = deterministic_answerer::try_answer("show gpu info", &context, &probes);

    assert!(answer.is_some());
    let answer = answer.unwrap();
    assert!(answer.contains("NVIDIA"));
    assert!(!answer.contains("VRAM"));
}

#[test]
fn test_no_gpu_detected() {
    let mut context = make_context();
    context.hardware.gpu = None;
    context.hardware.gpu_vram_gb = None;
    let probes = vec![];
    ...
    assert!(answer.is_some());
    let answer = answer.unwrap();
    assert!(answer.contains("No dedicated GPU detected"));
}

#[test]
fn test_query_classification_hardware() {
    ...
    assert_eq!(classify_query("tell me about my processor"), QueryType::CpuInfo);
    assert_eq!(classify_query("graphics card specs"), QueryType::GpuInfo);
}
