...
use crate::doctor_lifecycle::{
    ActionRisk, CollectedEvidence, DiagnosticCheck, DiagnosisFinding, DiagnosisResult,
    Doctor, ProposedAction, SafeNextStep,
};
use crate::doctor_registry::{DoctorDomain, FindingSeverity};
use crate::networking_doctor::{detect_network_manager, detect_manager_conflicts, NetworkManager};
...
impl NetworkingDoctorV2 {
    ...
    fn has_wireless_interface(evidence: &[CollectedEvidence]) -> bool {
        evidence.iter().any(|e| {
            e.data.get("interfaces")
                .and_then(|i| i.as_array())
                .map(|arr| arr.iter().any(|iface| {
                    iface.get("is_wireless").and_then(|w| w.as_bool()).unwrap_or(false)
                }))
                .unwrap_or(false)
        })
    }
    ...
    fn analyze_link(&self, evidence: &[CollectedEvidence]) -> Vec<DiagnosisFinding> {
        ...
        if let Some(iface_ev) = evidence.iter().find(|e| e.tool_name == "net_interfaces_summary") {
            if let Some(interfaces) = iface_ev.data.get("interfaces").and_then(|i| i.as_array()) {
                let up_count = interfaces.iter()
                ...
                for iface in interfaces {
                    ...
                    let carrier = iface.get("carrier").and_then(|c| c.as_bool()).unwrap_or(false);
                    ...
                    if carrier && !has_ip {
                        findings.push(DiagnosisFinding {
                            ...
                            description: format!("Interface {} has carrier but no IP address", name),
    ...
    fn analyze_routes(&self, evidence: &[CollectedEvidence]) -> Vec<DiagnosisFinding> {
        ...
        if let Some(route_ev) = evidence.iter().find(|e| e.tool_name == "net_routes_summary") {
            let has_default = route_ev.data.get("has_default_route")
    ...
    fn analyze_dns(&self, evidence: &[CollectedEvidence]) -> Vec<DiagnosisFinding> {
        ...
        if let Some(dns_ev) = evidence.iter().find(|e| e.tool_name == "dns_summary") {
            let servers = dns_ev.data.get("servers")
    ...
    fn analyze_connectivity(&self, evidence: &[CollectedEvidence]) -> Vec<DiagnosisFinding> {
        ...
        if let Some(ping_ev) = evidence.iter().find(|e| e.tool_name == "ping_check") {
            let success = ping_ev.data.get("success")
    ...
    fn analyze_wireless(&self, evidence: &[CollectedEvidence]) -> Vec<DiagnosisFinding> {
        ...
        if let Some(wifi_ev) = evidence.iter().find(|e| e.tool_name == "iw_summary") {
            let connected = wifi_ev.data.get("connected")
                ...
                "no_interfaces_up" => {
                    ...
                    if manager.manager == NetworkManager::NetworkManager {
                        actions.push(ProposedAction {
                            ...
                            rollback: Some(vec!["sudo systemctl restart NetworkManager".to_string()]),
                        });
                    }
                }
                "no_default_route" => {
...
impl Doctor for NetworkingDoctorV2 {
    ...
    fn domains(&self) -> Vec<&str> {
        vec!["network", "networking", "wifi", "ethernet", "internet", "dns", "ip"]
    }
    ...
    fn matches(&self, intent: &str, targets: &[String], raw_text: &str) -> u32 {
        ...
        if intent.contains("network") || intent.contains("diagnose") {
            if targets.iter().any(|t| {
                t.contains("network") || t.contains("internet") || t.contains("wifi")
            }) {
                return 95;
            }
        }
        ...
        let keywords = ["network", "internet", "wifi", "ethernet", "connected", "ip", "dns"];
    ...
    fn diagnose(&self, evidence: &[CollectedEvidence]) -> DiagnosisResult {
        ...
        let confidence = if all_findings.is_empty() {
            ...
            all_findings.iter().map(|f| f.confidence).max().unwrap_or(50)
        };
        ...
        let symptom_keywords: Vec<String> = all_findings.iter()
