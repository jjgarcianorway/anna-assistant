...
impl LlmContext {
    ...
    fn infer_system_identity(facts: &crate::types::SystemFacts) -> SystemIdentity {
        ...
        let classification = if let Some(behavior) = &facts.user_behavior {
            match behavior.user_profile.primary_use_case {
                ...
                crate::user_behavior::UseCase::Development => SystemClassification::DevelopmentWorkstation,
                crate::user_behavior::UseCase::ServerAdmin => SystemClassification::HomeServer,
                crate::user_behavior::UseCase::MediaProduction => SystemClassification::MediaProductionStation,
                crate::user_behavior::UseCase::Workstation => SystemClassification::GeneralPurposeDesktop,
                crate::user_behavior::UseCase::GeneralUse => SystemClassification::GeneralPurposeDesktop,
            }
        } else {
        ...
        let secondary_workloads = if let Some(behavior) = &facts.user_behavior {
            behavior.user_profile.secondary_use_cases.iter()
                .map(|uc| format!("{:?}", uc))
                .collect()
        } else {
            Vec::new()
        ...
        let hardware_tier = if facts.total_memory_gb >= 16.0 && facts.cpu_cores >= 8 && facts.is_nvidia {
    ...
    fn analyze_stability(facts: &crate::types::SystemFacts) -> StabilityIndicators {
        ...
        let filesystem_health_score = facts.filesystem_health.as_ref()
            .map(|fh| fh.health_score())
            .unwrap_or(100);

        overall_stability_score = overall_stability_score.saturating_sub(100 - filesystem_health_score);
        ...
        let backup_status = if let Some(backup) = &facts.backup_detection {
            match backup.overall_status {
                ...
                crate::backup_detection::BackupStatus::NoBackupTool => BackupStatus::NoToolInstalled,
            }
        } else {
    ...
    fn analyze_performance(facts: &crate::types::SystemFacts) -> PerformanceIndicators {
        ...
        let cpu_health = CpuHealth {
            throttling: facts.cpu_throttling.as_ref()
                .map(|ct| ct.throttling_events.has_throttling)
                .unwrap_or(false),
            governor_optimal: facts.cpu_performance.as_ref()
                .map(|cp| cp.governor.governor != "powersave")
                .unwrap_or(true),
            microcode_current: facts.cpu_performance.as_ref()
                .and_then(|cp| cp.microcode_version.as_ref())
                .is_some(),
            temperature_ok: facts.sensors_info.as_ref()
        ...
        let memory_health = MemoryHealth {
            swap_configured: facts.memory_usage_info.as_ref()
                .map(|mu| mu.swap.total_gb > 0.0)
                .unwrap_or(false),
            oom_events: facts.memory_usage_info.as_ref()
                .map(|mu| !mu.oom_events.is_empty())
                .unwrap_or(false),
            pressure_ok: facts.memory_usage_info.as_ref()
                .and_then(|mu| mu.memory_pressure.as_ref())
                .map(|mp| mp.some_avg10 < 50.0)
                .unwrap_or(true),
            utilization_healthy: facts.memory_usage_info.as_ref()
        ...
        let storage_health = StorageHealth {
            smart_ok: facts.storage_info.as_ref()
                .map(|si| !si.devices.is_empty())
                .unwrap_or(true),
            trim_enabled: facts.filesystem_info.as_ref()
        ...
        let network_health = NetworkHealth {
            latency_ok: true,
            packet_loss_ok: true,
            dns_ok: facts.network_config.as_ref()
                .map(|nc| !nc.dns_servers.is_empty())
                .unwrap_or(true),
            firewall_active: facts.security_info.as_ref()
                .map(|si| !matches!(si.firewall_status, crate::security::FirewallStatus::Inactive))
                .unwrap_or(false),
        };
        ...
        let gpu_health = if facts.gpu_vendor.is_some() {
            Some(GpuHealth {
                throttling: facts.gpu_throttling.as_ref()
                    .map(|_gt| false)  // Simplified check
                    .unwrap_or(false),
                driver_ok: facts.display_issues.as_ref()
                ...
                temperature_ok: facts.sensors_info.as_ref()
                    ...
                    .map(|temp| temp < 85.0)
                    .unwrap_or(true),
                compute_available: facts.gpu_compute.as_ref()
            ...
            if let Some(sec_patterns) = &behavior.security_patterns {
                if sec_patterns.failed_login_attempts > 20 {
                    ...
                    critical_issues.push(CriticalIssue {
                        ...
                        description: format!("{} failed login attempts detected", sec_patterns.failed_login_attempts),
    ...
    fn infer_user_goals(facts: &crate::types::SystemFacts) -> UserGoals {
        ...
        if let Some(behavior) = &facts.user_behavior {
            // Detected use cases
            detected_use_cases.push(format!("Primary: {:?}", behavior.user_profile.primary_use_case));
            ...
            if let Some(gaming) = &behavior.gaming_patterns {
                if gaming.is_gaming_system {
                    let is_powersave = facts.cpu_performance.as_ref()
                        .map(|cp| cp.governor.governor == "powersave")
                        .unwrap_or(false);
                    ...
                    if is_powersave {
                        optimization_opportunities.push(OptimizationOpportunity {
                            ...
                            recommendation: "Switch CPU governor to 'performance' for gaming".to_string(),
                        });
                    }
                }
            }
            ...
            if let Some(dev) = &behavior.development_patterns {
                if dev.is_development_system && dev.git_repositories > 0 {
                    workflow_improvements.push(WorkflowImprovement {
                        ...
                        current_state: format!("{} git repositories detected", dev.git_repositories),
            ...
            let learning_curve = match behavior.user_profile.experience_level {
                ...
                crate::user_behavior::ExperienceLevel::Intermediate => LearningCurve::IntermediateLearner,
    ...
    pub fn to_summary(&self) -> String {
        ...
        summary.push_str(&format!("System Identity: {:?} ({})\n",
            self.system_identity.classification,
            self.system_identity.primary_workload));

        summary.push_str(&format!("Stability Score: {}/100 ({:?} uptime health)\n",
            self.stability_indicators.overall_stability_score,
            self.stability_indicators.uptime_health));

        summary.push_str(&format!("Performance Score: {}/100\n",
            self.performance_indicators.overall_performance_score));

        summary.push_str(&format!("Risk Score: {}/100 (lower is better)\n",
